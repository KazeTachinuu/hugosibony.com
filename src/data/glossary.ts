...
ELF Header:
  ...
  },
  {
    slug: 'opcode',
    term: 'Opcode',
    expansion: 'operation code',
    kind: 'binary',
    summary:
      'The numeric byte that tells a CPU or virtual machine which instruction to execute; any operands follow it.',
    body: [
      'An instruction stream is just bytes. The first byte (the opcode) selects the operation; the bytes after it are the operands. Decoding means mapping each opcode back to its mnemonic and knowing how many operand bytes it consumes.',
      'Opcode-to-mnemonic assignments are fixed per architecture (x86 `0x90` is always `NOP`), so a disassembler can hardcode them. A CTF VM can instead shuffle the mapping at runtime and hand it to you, forcing an emulator that decodes by name rather than by value.',
    ],
    example: `34 00 0a   ->  MOV_IMM R0, 10
 |  |  |
 |  |  +-- operand: immediate 0x0a = 10
 |  +----- operand: register R0
 +-------- opcode 0x34 = MOV_IMM`,
    exampleLang: 'text',
    moreHref: 'https://en.wikipedia.org/wiki/Opcode',
    moreLabel: 'Wikipedia - Opcode',
  },
  {
    slug: 'cet-ibt',
    term: 'CET/IBT',
