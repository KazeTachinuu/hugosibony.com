---
title: "PW Crack 5"
cat: "chal"
ctf: "PicoCTF"
solved: true
categories: "General"
difficulty: "⭐"
description: "Dictionary attack with custom hash - optimizing search over a large wordlist."
date: 2024-05-12T00:00:00Z
updated: 2026-07-13
draft: false
files:
  - { name: "level5.py", path: "/writeups/picoctf/2022/general_skills/pw_crack_5/level5.py" }
  - { name: "level5.flag.txt.enc", path: "/writeups/picoctf/2022/general_skills/pw_crack_5/level5.flag.txt.enc" }
  - { name: "level5.hash.bin", path: "/writeups/picoctf/2022/general_skills/pw_crack_5/level5.hash.bin" }
  - { name: "dictionary.txt", path: "/writeups/picoctf/2022/general_skills/pw_crack_5/dictionary.txt" }
  - { name: "solve.py", path: "/writeups/picoctf/2022/general_skills/pw_crack_5/solve.py" }
---

# PW Crack 5   

<Callout type="info" title="Challenge Information">

**Category:** General Skills  
**Points:** 100  
**Solves:** 24106   

**Description:**   
Can you crack the password to get the flag? Download the password checker here and you'll need the encrypted flag and the hash in the same directory too. Here's a dictionary with all possible passwords based on the password conventions we've seen so far.

</Callout>

<Callout type="info" title="Challenge Overview">

For the fifth and final PW Crack, the idea is the same and our implementation still works. This time we just format the output so the printed results look like a small "animation".

Let's also modify a bit the code so that it will get the passwords from the txt file.

</Callout>

<Callout type="success" title="Solution Implementation">

```python
content = open("dictionary.txt", "r").read()

pos_pw_list = content.split("\n")

i = 0
n = len(pos_pw_list)

print("Trying to match each password with provided hash")
hashed_pw = hash_pw(pos_pw_list[i])
while i < n and hashed_pw != correct_pw_hash:
    sys.stdout.write(
        f"\r{i+1}/{n} - {pos_pw_list[i]} {hashed_pw} != {correct_pw_hash}"
    )
    i += 1
    hashed_pw = hash_pw(pos_pw_list[i])
    
print()

if i < n:
    print("Password Matching Hash found ! : " + pos_pw_list[i])
    decryption = str_xor(flag_enc.decode(), pos_pw_list[i])
    print(decryption)

else:
    print("No password in the list matched the hash")
```
   
When we run it we get a nice one line that test every password hash:

```sh
>>> python solve.py
Trying to match each password with provided hash
38273/65537 - 9580 b'\x0c\xdb\xb4\xe6X\x15\xfb\xafyh\x9b\x15H.uu' != b'\x126P\xdd\x05`Xy\x18\xb3\xd7q\xcf\x0c\x01q'x0c\x01q'\x01q'1q'1q'
Password Matching Hash found ! : 9581
picoCTF{h45h_sl1ng1ng_36e992a6}
```

That wraps up the PW Crack series.

</Callout>

<Callout type="success" title="Flag">

`picoCTF{h45h_sl1ng1ng_36e992a6}`

</Callout>