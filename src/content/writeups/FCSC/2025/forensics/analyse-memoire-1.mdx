...
Un agent du FCSC démarre son ordinateur pour réfléchir et noter des idées de challenges pour l'année prochaine. Il nous a cependant alertés que lors du démarrage, un étrange écran rouge est apparu à peine une seconde, puis le poste a démarré normalement.
...
Notre objectif est d'analyser la mémoire et de retrouver le malware qui tente d'exfiltrer le document.

</Callout>

<Callout type="note" title="1. Analyse préliminaire de l'image mémoire" icon="search">
...
On utilise <Term slug="volatility">Volatility 3</Term>, un outil d'analyse forensique de la mémoire RAM, pour obtenir des informations sur le système :
...
<Callout type="note" title="2. Recherche des connexions réseau actives" icon="network-wired">
...
<Callout type="note" title="3. Identification du processus malveillant" icon="lightbulb">
...
#### Pourquoi `rundll32.exe` est-il suspect ?
...
<Flag flag="FCSC{rundll32.exe:1800:100.68.20.103:443:TCP}" />
